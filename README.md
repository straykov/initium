...
##Режимы
Одноразовая сборка (с минификацией, для продакшена):
```bash
npm start
```
```bash
...
Запуск живой сборки на локальном сервере (без минификации, для теста):
```bash
npm run live
```
```bash
...
Сборка без автоматической перезагрузки страниц (без минификации, для теста):
```bash
npm run no-server
```
```bash
...
Живая сборка на локальном сервере и туннель в интернет (без минификации, для теста):
```bash
